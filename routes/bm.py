...
class DataFetch:
    ...
    def getReviews( self, page ):
        route = f'https://{ENV_TARGET}/bm/reviews/v2/product/{self.uuid}/list?page={page}'
        return DataFetch.request( route, self.headers, {"http": self.proxy} )

    def getOffers( self ):
        route = f'https://{ENV_TARGET}/bm/product/{self.uuid}/v3/best_offers'
        return DataFetch.request( route, self.headers, {"http": self.proxy} )

    def getTechnicalSpecs( self ):
        route = f'https://{ENV_TARGET}/bm/product/{self.uuid}/technical_specifications'
        return DataFetch.request( route, self.headers )

    def getVariants( self ):
        route = f'https://{ENV_TARGET}/bm/product/{self.uuid}/v2/variants'
        return DataFetch.request( route, self.headers )
        
    def getGradeDescriptions( self ):
        route = f'https://{ENV_TARGET}/bm/product/{self.uuid}/grade_descriptions?include_technical_comment=tru'
